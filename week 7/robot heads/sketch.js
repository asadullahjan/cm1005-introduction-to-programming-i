...
function setup()
{
	...
	robot2Speed_x = random(2,3);
	robot2Speed_y = random(2,3);

	console.log((floor("25.123")  + round("25.123")));
	console.log(Number(25.123) * 2);
	numWays = '25.123'
	console.log(round(numWays + numWays));
	console.log(floor(numWays * 2));
	console.log(round(numWays * 2));
	console.log(floor(25.623));
}
...
function draw()
{
	...
	if(robot1_x >= width || robot1_x <= 0){
		robot1Speed_x *= -1;
	}
	if(robot1_y >= height || robot1_y <= 0 ){
		robot1Speed_y *= -1;
	}


	if(robot2_x >= width || robot2_x <= 0){
		robot2Speed_x *= -1;
	}
	if(robot2_y >= height || robot2_y <= 0 ){
		robot2Speed_y *= -1;
	}

	bounceWithOneAnother();
	
}

function bounceWithOneAnother() {
	distanceX = abs(robot1_x - robot2_x);
	distanceY = abs(robot1_y - robot2_y);
